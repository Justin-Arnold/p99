...
func runProfile(args []string, stdout, stderr io.Writer) int {
	...
	var probeSpecPath, probeBaseURL string
	var seed int64
	...
	fs.StringVar(&probeSpecPath, "probe-spec", "", "YAML or JSON request spec to probe while the profile is captured")
	fs.StringVar(&probeBaseURL, "probe-base-url", "", "override probe request spec base_url")
	fs.Int64Var(&seed, "seed", 0, "seed for probe request spec randomization; 0 generates one")
	...
	valueFlags := map[string]bool{
		...
		"probe-spec": true, "probe-base-url": true, "seed": true,
	...
	if probeURL != "" && probeSpecPath != "" {
		fmt.Fprintln(stderr, "--probe cannot be used with --probe-spec")
		return 2
	}
	if probeSpecPath == "" {
		explicit := explicitFlags(fs)
		for _, name := range []string{"probe-base-url", "seed"} {
			if explicit[name] {
				fmt.Fprintf(stderr, "--%s requires --probe-spec\n", name)
				return 2
			}
		}
	}
	if probeURL != "" || probeSpecPath != "" {
		...
		if probeSpecPath != "" {
			plan, err = loadRequestPlan(probeSpecPath, probeBaseURL, seed, explicitFlags(fs)["seed"], &probeCfg)
			if err != nil {
				fmt.Fprintf(stderr, "probe spec: %v\n", err)
				return 2
			}
		}
		...
		go func() {
			result, err := probe.HTTPRunner{Config: probeCfg, RequestPlan: plan}.Run(ctx)
